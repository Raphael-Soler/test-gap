...
void test_cluster_dma(void)
{
    ...
    uint32_t buffer_size = size*size;//nb_cl_pe_cores * (uint32_t) BUFFER_SIZE_PER_CORE;
...
*/    
    ...
    unsigned short filter[9] = {-1, -2, -1, 0, 0, 0, 1, 2, 1}; //possibly an error here because it should be impossible to have negative value in the unsigned format
    unsigned short result_conv[size*size];
    ...
    for (int i = 1; i < size-1; i++) 
    {
        for (int j = 1; j < size-1; j++) 
        {
            ...
            for (int i = -1; i <= 1; i++) 
            {
            	for (int j = -1; j <= 1; j++) 
            	{
            	int neighborIndex = index + (i * (size)) + j;
            	result += l2_out_matrice2[neighborIndex] * filter[k];
            	k++;
            	}
            }
            result_conv[index] = result;
        }
    }

    //Print the convolution matrix result
    printf("Matrix convolution :\n");
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            int index = (i * size) + j;
            printf("%i ", result_conv[index]);
        }
        printf("\n");
    }
